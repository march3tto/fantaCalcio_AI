          ...
          {filtrati.map((g) => (
            <div key={g.id} className="flex items-center justify-between bg-panel border border-line rounded-lg px-3 py-2">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button onClick={() => rimuoviGiocatore(g.id)} className="text-inkdim p-1"><Trash2 size={14} /></button>
              </div>
            </div>
          ))}
        </div>
    ...
    <div className="space-y-4">
      <div className="flex gap-2">
        <div className="relative flex-1">
          <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-inkdim" />
          <input
            ref={inputRef}
            value={query}
          ...
          />
          {query && !selezionato ? (
            <button onClick={reset} className="absolute right-3 top-1/2 -translate-y-1/2 text-inkdim"><X size={16} /></button>
          ) : !selezionato ? (
            <button
      ...
      {selezionato && (
        <div>
          ...
          <div className="relative mb-2">
            ...
            />
            <Mic size={15} className="absolute right-3 top-1/2 -translate-y-1/2 text-inkdim pointer-events-none" />
          </div>
      ...
      {selezionato && analisi && (
        <div className="bg-panel border border-line rounded-xl p-4 space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={reset} className="text-inkdim p-1 shrink-0"><X size={18} /></button>
          </div>
          ...
          {impattoRosa && (
            <div className="bg-inkbg border border-line rounded-lg p-2.5 space-y-1.5">
              <div className="flex items-center justify-between">
                <span className="flex items-center gap-1.5 text-[10px] text-inkdim uppercase"><Users size={12} /> Impatto rosa · {setup.modulo} · {ruoloScelto}</span>
              ...
              return (
                <div key={r.giocatoreId} className="flex items-center justify-between rounded-lg px-3 py-2 border bg-panel border-line">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button onClick={() => annullaAssegnazione(ioSquadra.id, r.giocatoreId)} className="text-inkdim p-1"><Undo2 size={14} /></button>
                  </div>
                </div>
              );
            })}
